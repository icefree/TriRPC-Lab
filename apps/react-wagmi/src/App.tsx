...
const SEPOLIA_COUNTER_ADDRESS = '0xA327E3966De7B3E7EA204C69094479d996fd4849'
...
function methodLibraryMapping(method: string): { ethers: string; viem: string; wagmi: string } {
  const map: Record<string, { ethers: string; viem: string; wagmi: string }> = {
    eth_chainId: {
      ...
      wagmi: 'useChainId() / getChainId(config)',
    },
    eth_blockNumber: {
      ethers: 'provider.getBlockNumber()',
      viem: 'publicClient.getBlockNumber()',
      wagmi: 'useBlockNumber() / getBlockNumber(config)',
    },
    eth_getBalance: {
      ethers: 'provider.getBalance(address)',
      viem: 'publicClient.getBalance({ address })',
      wagmi: 'useBalance({ address }) / getBalance(config, { address })',
    },
    eth_getTransactionCount: {
      ethers: 'provider.getTransactionCount(address)',
      viem: 'publicClient.getTransactionCount({ address })',
      wagmi: 'useTransactionCount({ address }) / getTransactionCount(config, { address })',
    },
    eth_getCode: {
      ethers: 'provider.getCode(address)',
      viem: 'publicClient.getCode({ address })',
      wagmi: 'useBytecode({ address }) / getBytecode(config, { address })',
    },
    eth_getStorageAt: {
      ethers: 'provider.getStorage(address, slot)',
      viem: 'publicClient.getStorageAt({ address, slot })',
      wagmi: 'useStorageAt({ address, slot }) / getStorageAt(config, { address, slot })',
    },
    eth_getBlockByNumber: {
      ethers: 'provider.getBlock(blockNumber)',
      viem: 'publicClient.getBlock({ blockNumber | blockTag })',
      wagmi: 'useBlock({ blockNumber | blockTag }) / getBlock(config, ...)',
    },
    eth_getBlockByHash: {
      ethers: 'provider.getBlock(blockHash)',
      viem: 'publicClient.getBlock({ blockHash })',
      wagmi: 'useBlock({ blockHash }) / getBlock(config, ...)',
    },
    eth_getBlockTransactionCountByNumber: {
      ethers: "provider.send('eth_getBlockTransactionCountByNumber', ...)",
      viem: 'publicClient.getBlockTransactionCount({ blockNumber | blockTag })',
      wagmi: 'useBlockTransactionCount({ blockNumber | blockTag }) / getBlockTransactionCount(config, ...)',
    },
    ...
    personal_sign: {
      ...
      wagmi: 'useSignMessage() / signMessage(config, ...)',
    },
    personal_ecRecover: {
    ...
    eth_signTypedData_v4: {
      ...
      wagmi: 'useSignTypedData() / signTypedData(config, ...)',
    },
    eth_requestAccounts: {
      ethers: "provider.send('eth_requestAccounts', [])",
      viem: 'walletClient.requestAddresses()',
      wagmi: 'useConnect() / connect(config, ...)',
    },
    eth_accounts: {
      ethers: 'provider.listAccounts()',
      viem: 'walletClient.getAddresses()',
      wagmi: 'useAccount() / getAccount(config)',
    },
    wallet_switchEthereumChain: {
      ethers: "provider.send('wallet_switchEthereumChain', [{ chainId }])",
      viem: 'walletClient.switchChain({ id })',
      wagmi: 'useSwitchChain() / switchChain(config, ...)',
    },
    wallet_addEthereumChain: {
      ethers: "provider.send('wallet_addEthereumChain', [chain])",
      viem: 'walletClient.addChain({ chain })',
      wagmi: 'useWalletClient().data?.addChain({ chain })',
    },
    wallet_watchAsset: {
      ethers: "provider.send('wallet_watchAsset', [params])",
      viem: 'walletClient.watchAsset({ type, options })',
      wagmi: 'useWatchAsset() / watchAsset(config, ...)',
    },
    wallet_getPermissions: {
      ethers: "provider.send('wallet_getPermissions', [])",
      viem: 'walletClient.getPermissions()',
      wagmi: 'useWalletClient().data?.getPermissions()',
    },
    wallet_requestPermissions: {
      ethers: "provider.send('wallet_requestPermissions', [params])",
      viem: 'walletClient.requestPermissions({ eth_accounts: {} })',
      wagmi: 'useWalletClient().data?.requestPermissions({ eth_accounts: {} })',
    },
    ...
    wallet_getCapabilities: {
      ...
      viem: 'walletClient.getCapabilities({ chainId })',
      wagmi: 'useCapabilities({ account, chainId }) / getCapabilities(config, ...)',
    },
    wallet_sendCalls: {
      ethers: "provider.send('wallet_sendCalls', [params])",
      viem: 'walletClient.sendCalls({ calls })',
      wagmi: 'useSendCalls() / sendCalls(config, ...)',
    },
    wallet_getCallsStatus: {
      ethers: "provider.send('wallet_getCallsStatus', [id])",
      viem: 'walletClient.getCallsStatus({ id })',
      wagmi: 'useCallsStatus({ id }) / useWaitForCallsStatus({ id })',
    },
    eth_subscribe: {
      ethers: 'provider.on(event, listener)',
      viem: 'watchBlocks()/watchEvent()/watchPendingTransactions()',
      wagmi: 'useWatchBlocks()/useWatchContractEvent()/useWatchPendingTransactions()',
    },
    eth_unsubscribe: {
      ethers: 'provider.off(...)',
      viem: 'unwatch()',
    ...
    newHeads: {
      ...
      viem: 'watchBlocks(...)',
      wagmi: 'useWatchBlockNumber(...) / useWatchBlocks(...)',
    },
    logs: {
    ...
    newPendingTransactions: {
      ...
      wagmi: 'useWatchPendingTransactions(...)',
    },
  ...
  return map[method] ?? { ethers: '', viem: '', wagmi: '' }
}
...
function App() {
  ...
  useEffect(() => {
    if (chainId === SEPOLIA_CHAIN_ID) {
      setContractInput(SEPOLIA_COUNTER_ADDRESS)
    }
  }, [chainId])
